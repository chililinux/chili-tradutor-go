...
func main() {
	...
	// Resolve o nome base para evitar problemas com caminhos absolutos
	...
	// Cabeçalho com alinhamento e cores corrigidas
...
// --- Funções de Processamento Ajustadas para caminhos ---
	...
	for scanner.Scan() {
		...
		if strings.HasPrefix(line, "msgid ") {
			...
			if !quietFlag && !verboseFlag {
				fmt.Printf("\r   %s %-7s [%d/%d] %d%% ", color.BlueString("→"), cyan(lang), current, totalLines, (current*100)/totalLines)
			}
...
func translateMessage(msgid, langCode string) string {
	...
	if _, ok := cacheData[langCode]; !ok {
		cacheData[langCode] = make(map[string]string)
	}
