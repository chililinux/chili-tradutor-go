/*
    ...
    Updated:   qui 15 jan 2026 23:30:00 -04
    Version:   2.1.14
*/
...
var defaultLanguages = []string{"pt-BR", "en", "es", "it", "de", "fr", "ru", "zh-CN", "zh-TW", "ja", "ko"}

// --- FUNÇÃO DE EXECUÇÃO COM ISOLAMENTO DE LOCALE ---

func execCommand(name string, args ...string) *exec.Cmd {
	cmd := exec.Command(name, args...)
	// Força ambiente neutro para evitar que o idioma do sistema (ex: pt_BR) interfira
	// na lógica das ferramentas de tradução (xgettext, msginit, etc)
...
func callUniversalTranslator(text, lang string) string {
	...
	var err error
	for i := 0; i < 3; i++ {
		// Motor universal via translate-shell
		cmd := execCommand("trans", "-e", engine, "-s", sourceLang, "-no-init", "-no-autocorrect", "-b", ":"+lang)
...
func writeMsgfmtToMo(base, lang string) {
	...
	execCommand("msgfmt", poFile, "-o", moFile).Run()
}
