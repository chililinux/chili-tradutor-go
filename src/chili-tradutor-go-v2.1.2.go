/*
    ...
    Updated:   seg 12 jan 2026 10:20:00 -04
    Version:   2.1.2
...
func main() {
	...
	fmt.Printf("   %s %-15s: %s (%s)\n", green("→"), white("Cache Hits"), green(fmt.Sprintf("%-5d", cacheHits)), green(fmt.Sprintf("%.2f%%", pCache)))
	fmt.Printf("   %s %-15s: %s (%s)\n", yellow("→"), white("Chamadas Rede"), yellow(fmt.Sprintf("%-5d", netCalls)), yellow(fmt.Sprintf("%.2f%%", pNet)))
	fmt.Printf("   %s %-15s: %s\n", blue("→"), white("Total Strings"), white(fmt.Sprintf("%-5d", totalCalls)))
	fmt.Println()
}
