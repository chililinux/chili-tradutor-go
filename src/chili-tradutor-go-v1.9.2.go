...
func main() {
	...
	if isMarkdown {
		fmt.Printf("%s %s\n", yellow("[STEP 1]"), white("Modo Markdown Detectado."))
	} else {
		fmt.Printf("%s %s\n", yellow("[STEP 1]"), white("Limpando e extraindo strings para pot/ ..."))
		// LIMPEZA DO POT ANTIGO PARA GARANTIR NOVA GERAÇÃO
...
func prepareMsginit(baseName, lang string) {
	...
	os.Remove(poTmp) // Limpa temporário antigo
...
func translateFile(baseName, lang string) {
	...
	for scanner.Scan() {
		...
		if strings.HasPrefix(line, "msgid ") {
			...
			current++
			if !quietFlag && !verboseFlag {
				// Mostra o progresso na mesma linha usando \r
	...
	// O SEGREDO ESTÁ AQUI: 
	// Ao terminar um idioma, damos um print com \n para "fixar" a linha no terminal
	// e não deixar o próximo idioma ou o resumo sobrescrever ela.
	if !quietFlag && !verboseFlag {
		fmt.Printf("\n") // Fixa a linha do idioma que acabou de completar
	}
...
func writeMsgfmtToMo(baseName, lang string) {
	...
	// Verifica se o arquivo PO existe antes de rodar o compilador
	if _, err := os.Stat(poFinal); err == nil {
		cmd := exec.Command("msgfmt", poFinal, "-o", moFile)
		out, err := cmd.CombinedOutput()
		if err != nil && verboseFlag {
			fmt.Printf("\n%s Erro msgfmt (%s): %v\n%s", red("[!]"), lang, err, string(out))
		}
	}
}
...
func usage() {
	...
	fmt.Printf("  %s, %-15s %s\n", yellow("-j"), yellow("--jobs"), white("Paralelos (Padrão: 8)"))
	fmt.Printf("  %s, %-15s %s\n", yellow("-f"), yellow("--force"), white("Forçar tradução (Ignorar cache)"))
	fmt.Printf("  %s, %-15s %s\n", yellow("-v"), yellow("--verbose"), white("Modo detalhado"))
