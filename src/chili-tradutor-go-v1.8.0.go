...
var (
	...
	netCalls     int
	isOnline     bool
	langsDone    int32 // Contador atômico para o Passo 3
)
...
func main() {
	...
	// [PASSO 3] FEEDBACK GLOBAL COM PROGRESSO
	...
	for _, lang := range targetLangs {
		...
		go func(l string) {
			...
			if !quietFlag && !verboseFlag {
				current := atomic.LoadInt32(&langsDone)
				fmt.Printf("\r   %s %s: %d/%d concluídos...", blue("→"), white("Progresso"), current, totalLangs)
			}
		}(lang)
	}
	wg.Wait()
