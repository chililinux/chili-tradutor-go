...
func main() {
	...
	if isMarkdown {
		fmt.Printf("%s %s\n", yellow("[STEP 1]"), white("Modo Markdown: Saída em doc/ com sufixo e proteção de links"))
...
func protectVariables(text string) (string, map[string]string) {
	// REGEX ATUALIZADO: Protege Variáveis Shell, Links Markdown E Tags HTML <a>
	re := regexp.MustCompile(`(\$\{[A-Za-z0-9_.]+\}|\$[A-Za-z0-9_.]+|%[a-z]|\[.*?\]\(.*?\)|<a\s+.*?>.*?</a>)`)
