...
func main() {
	...
	if isMarkdown {
		fmt.Printf("%s %s\n", yellow("[STEP 1]"), white("Modo Markdown: Saída em doc/ com Blindagem de Links"))
...
func callUniversalTranslator(text, lang string) string {
	...
	// -no-init impede trans de abrir navegadores. 
	// Passamos via Stdin para evitar problemas de shell escape.
...
func protectVariables(text string) (string, map[string]string) {
	// REGEX REFORÇADA:
	// 1. Variáveis Shell: ${VAR} ou $VAR
	// 2. Links Markdown: [texto](url)
	// 3. Tags HTML <a>: <a ...>...</a>
	// 4. URLs puras: http://... ou https://...
