...
// Idiomas padrão (Top 5 - de, it, en, es, fr)
...
func usage() {
	...
	fmt.Printf("%s\n", boldred("OPÇÕES:"))
	fmt.Printf("  %s, %-15s %s\n", yellow("-i"), yellow("--inputfile"), white("Arquivo de entrada (ex: void-install)"))
	fmt.Printf("  %s, %-15s %s %s\n", yellow("-l"), yellow("--language"), white("Idiomas separados por vírgula ou"), magenta("'all'"))
	fmt.Printf("  %s, %-15s %s %s\n", yellow("-e"), yellow("--engine"), white("Motor: google, bing, yandex (padrão:"), cyan("google") + white(")"))
	fmt.Printf("  %s, %-15s %s\n", yellow("-j"), yellow("--jobs"), white("Número de traduções simultâneas (padrão: 3)"))
	fmt.Printf("  %s, %-15s %s\n", yellow("-f"), yellow("--force"), white("Força a tradução mesmo que já exista no cache"))
	fmt.Printf("  %s, %-15s %s\n", yellow("-v"), yellow("--verbose"), white("Mostra detalhadamente cada ID traduzido"))
	fmt.Printf("  %s, %-15s %s\n", yellow("-q"), yellow("--quiet"), white("Modo silencioso (logs apenas no arquivo)"))
	fmt.Printf("  %s, %-15s %s\n", yellow("-V"), yellow("--version"), white("Mostra a versão do programa"))
	fmt.Printf("  %s, %-15s %s\n\n", yellow("-h"), yellow("--help"), white("Mostra esta tela de ajuda"))

	fmt.Printf("%s\n", boldred("EXEMPLOS:"))
	fmt.Printf("  %s -i void-install\n", cyan(_APP_))
	fmt.Printf("  %s -i void-install -l %s\n", cyan(_APP_), magenta("en,es,it"))
	fmt.Printf("  %s -i void-install -l %s -j 8\n\n", cyan(_APP_), magenta("all"))
}
