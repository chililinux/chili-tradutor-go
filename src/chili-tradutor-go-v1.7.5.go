...
var defaultLanguages = []string{"en", "es", "it", "de", "fr", "ru", "zh_CN", "zh_TW", "ja", "ko"}
...
func main() {
	...
	if !isOnline { connStatus = red("OFFLINE (Modo Cache)") }
	...
	fmt.Printf("%s %-20s %s\n", magenta("[ENGINE]"), white("Motor:"), cyan(engine)) // <-- Adicionado aqui
	fmt.Printf("%s %-20s %d\n", magenta("[JOBS]"), white("Paralelos:"), jobs)
	fmt.Println()
...
func translateFile(inputFile, lang string) {
	...
	// Conta total de linhas para o progresso
	countScanner := bufio.NewScanner(file)
	totalLines := 0
	for countScanner.Scan() { if strings.HasPrefix(countScanner.Text(), "msgid ") { totalLines++ } }
	file.Seek(0, 0)
	...
	for scanner.Scan() {
		...
		if strings.HasPrefix(line, "msgid ") {
			...
			current++
			// Barra de progresso visual simplificada por idioma
			if !quietFlag && !verboseFlag {
				fmt.Printf("\r   %s %-6s [%d/%d] %d%% ", blue("→"), cyan(lang), current, totalLines, (current*100)/totalLines)
			}
...
// Funções de apoio (as mesmas das anteriores, mantendo a integridade)
func translateMessage(msgid, langCode string) string {
	...
	mu.Unlock()
	if !isOnline { return formatMsgstr(cleanID) }
