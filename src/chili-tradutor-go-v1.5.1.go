...
func main() {
	...
	if err := checkEngineHealth(engine); err != nil {
		logger.Printf("%s Motor %s offline, usará fallback se necessário.\n", yellow("[WARN]"), engine)
	}

	logger.Printf("%s Extraindo strings...\n", black("[STEP 1]"))
	prepareGettext(inputFile)

	// MENSAGENS INDIVIDUAIS POR PARÂMETRO
	...
	fmt.Println() // Linha em branco para separar do log de cada idioma
	...
	wg.Wait()
	fmt.Printf("\n%s %s\n", green("✔"), "Todas as traduções foram concluídas!")
}
...
func getFromCache(lang, text string) (string, bool) {
	...
	if !forceFlag {
		if l, ok := cacheData[lang]; ok {
			if t, ok := l[text]; ok { return t, true }
		}
	}
	return "", false
}
...
func translateMessage(msgid, lang, eng string) string {
	...
	if val, ok := getFromCache(lang, msgid); ok { return formatMsgstr(val) }
...
func checkEngineHealth(eng string) error {
	return exec.Command("trans", "-e", eng, "-no-autocorrect", "-b", ":en", "health").Run()
}
...
func usage() {
	...
	fmt.Println("  -l, --language    Idiomas (ex: pt-BR,en) ou 'all' para os 32")
	...
	fmt.Println("  -q, --quiet       Modo silencioso")
	fmt.Println("  -V, --version     Versão")
