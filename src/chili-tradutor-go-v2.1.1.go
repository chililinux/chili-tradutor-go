/*
    ...
    Updated:   seg 12 jan 2026 10:15:00 -04
    Version:   2.1.1
...
func main() {
	...
	// STEP 2 - RESTAURADO: Idiomas, Motor, Jobs e REDE
	...
	for _, lang := range targetLangs {
		...
		go func(l string) {
			...
			muConsole.Lock()
			fmt.Printf("\r   %s %s / %s tarefas concluídas...", yellow("[STATUS]"), green(langsDone), green(totalLangs))
			muConsole.Unlock()
	...
	fmt.Printf("   %s %s: %s\n", green("→"), white("Cache Hits   "), green(cacheHits))
	fmt.Printf("   %s %s: %s\n", yellow("→"), white("Consultas Net"), yellow(netCalls))
	fmt.Println()
}
...
func callUniversalTranslator(text, lang string) string {
	...
	// Se estiver offline e não achou no cache, retorna o original
...
func checkInternet() bool {
	// Tenta conectar no Google Port 80 com timeout curto de 2 segundos
