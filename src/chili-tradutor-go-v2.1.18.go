/*
    ...
    Updated:   qua 28 jan 2026 09:40:00 -04
    Version:   2.1.18
*/
...
func callUniversalTranslator(text, lang string) string {
	...
	// Normaliza para o comando 'trans' (ex: pt_BR -> pt-BR)
...
func detectFileType(path string) (ext string, lang string, desc string) {
	...
	if ext == "" {
		...
		if detected != "" && detected != "shell" {
			return "", detected, fmt.Sprintf(T("Script (%s)"), green(detected))
		}
		// Se não tem extensão e não detectou shebang específico, assume TXT
...
func hasActualContent(ext, baseName string) bool {
	...
	if ext == ".md" || ext == ".markdown" || ext == ".txt" || ext == ".json" || ext == ".yaml" || ext == ".yml" || ext == ".html" || ext == ".htm" {
		return true
	}
