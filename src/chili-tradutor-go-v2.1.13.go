/*
    ...
    Updated:   qua 14 jan 2026 01:05:00 -04
    Version:   2.1.13
*/
...
func TN(msgid, msgidPlural string, n int) string {
	cmd := exec.Command("ngettext", "-d", _APP_, msgid, msgidPlural, fmt.Sprintf("%d", n))
	out, err := cmd.Output()
	if err != nil {
		if n == 1 {
			return msgid
...
func runFullSelfTest() {
	...
	// 4. Teste YAML/JSON
	fmt.Printf("    %s %-35s ", blue("→"), T("Validação de YAML/JSON"))
	ymlFile := "test_integrity.yml"
	_ = os.WriteFile(ymlFile, []byte("key: \"Value\""), 0644)
	os.MkdirAll("yml", 0755)
	translateJSON(ymlFile, "fr")
	if _, err := os.Stat("yml/test_integrity-fr.yml"); err == nil {
		fmt.Println(green("OK"))
		os.Remove(ymlFile)
		os.Remove("yml/test_integrity-fr.yml")
	} else {
		fmt.Println(red("FALHA"))
	}

	// 5. Teste Scripts (Extração)
	fmt.Printf("    %s %-35s ", blue("→"), T("Extração de Scripts (SH/PY)"))
	pyFile := "test_integrity.py"
	_ = os.WriteFile(pyFile, []byte("print(gettext(\"Hello\"))"), 0644)
	os.MkdirAll("pot", 0755)
	prepareGettext(pyFile, pyFile, "python")
	if _, err := os.Stat("pot/test_integrity.pot"); err == nil {
		fmt.Println(green("OK"))
		os.Remove(pyFile)
		os.Remove("pot/test_integrity.pot")
	} else {
		fmt.Println(red("FALHA"))
	}

	// 6. Stress de Concorrência
	fmt.Printf("    %s %-35s ", blue("→"), T("Stress de Concorrência (Async)"))
	var wg sync.WaitGroup
	for i := 0; i < 15; i++ {
		wg.Add(1)
		go func(n int) {
			defer wg.Done()
			_ = callUniversalTranslator(fmt.Sprintf("Stress Test %d", n), "de")
		}(i)
	}
	wg.Wait()
	fmt.Println(green("OK"))
	...
	// 7. Tradução Real
	fmt.Printf("    %s %-35s ", blue("→"), T("Tradução Real (Network)"))
	if isOnline {
		if res := callUniversalTranslator("Integrity Test", "it"); res != "" {
			fmt.Println(green("OK"))
		} else {
			fmt.Println(yellow("TIMEOUT/EMPTY"))
		}
	} else {
		fmt.Println(yellow("OFFLINE"))
	}
...
func stampPotHeader(potPath string) {
	header := fmt.Sprintf(
		"# Chili Tradutor Go - %s\n"+
		...
		_VERSION_, _APP_, _APP_, _VERSION_, time.Now().Format("2006-01-02 15:04-0700"),
	)
	...
	if found {
		...
		os.WriteFile(potPath, []byte(final), 0644)
	}
}
...
func prepareGettext(inputPath, baseName, lang string) {
	...
	exec.Command("xgettext", "--from-code=UTF-8", "--language="+lang, "--keyword=gettext", "--keyword=_", "--keyword=T", "--keyword=TN:1,2", "--force-po", "-o", pot, inputPath).Run()
	stampPotHeader(pot)
}

func prepareGettextSelf(inputPath string) {
	pot := filepath.Join("pot", _APP_+".pot")
	exec.Command("xgettext", "--from-code=UTF-8", "--keyword=T", "--keyword=TN:1,2", "--no-wrap", "-o", pot, inputPath).Run()
	stampPotHeader(pot)
}
...
// translateHTML atualizada para feedback linha por linha
func translateHTML(inputPath, lang string) {
	...
	for i, line := range lines {
		...
		// Proteção de tags HTML na linha
		...
		// Traduz apenas se houver algo além de tags
...
func checkDependencies() {
	...
	fmt.Printf(" %s %s\n", yellow(" →"), T("Comandos de instalação sugeridos:"))

	fmt.Printf("\n    %s: %s\n", cyan("Chili Linux"), green("sudo pacman -S "+pkgList))
	fmt.Printf("    %s: %s\n", cyan("Arch Linux"), green("sudo pacman -S "+pkgList))
	fmt.Printf("    %s: %s\n", cyan("Void Linux"), green("sudo xbps-install -S "+pkgList))
	fmt.Printf("    %s: %s\n", cyan("Debian / Ubuntu"), green("sudo apt install "+pkgList))
	fmt.Printf("    %s: %s\n", cyan("Fedora / RHEL"), green("sudo dnf install "+pkgList))

	distro := detectDistro()
	installCmd := ""

	switch distro {
	case "chili", "chililinux", "arch":
		installCmd = "sudo pacman -S " + pkgList
	case "void":
		installCmd = "sudo xbps-install -S " + pkgList
	case "debian", "ubuntu":
		installCmd = "sudo apt install " + pkgList
	case "fedora":
		installCmd = "sudo dnf install " + pkgList
	}
	...
	if installCmd != "" {
		...
		if strings.ToLower(response) == "s" {
			...
			if err := cmd.Run(); err == nil {
				fmt.Println(green(T("\nDependências instaladas! Reiniciando...\n")))
				return
			}
		}
	} else {
	...
	fmt.Println(yellow(T("\nPor favor, instale as dependências manualmente e tente novamente.")))
	os.Exit(1)
}
...
func hasActualContent(ext, baseName string) bool {
	...
	if ext == ".md" || ext == ".markdown" || ext == ".json" || ext == ".yaml" || ext == ".yml" || ext == ".html" || ext == ".htm" {
		return true
	}
...
func usage() {
    ...
    flags := []struct{ short, long, desc string }{
        {"-i", "--inputfile", T("Arquivo fonte (.sh, .py, .md, .json, .yaml, .html, .pot)")},
    ...
    fmt.Fprintf(os.Stderr, "\n%s:\n  %s\n\n", yellow(T("Exemplos")), green("chili-tradutor-go -i script.sh -l en,es"))
}
