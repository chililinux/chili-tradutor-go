...
var (
	...
	netCalls     int
	isOnline     bool
	langsDone    int32 // Contador atômico
)
...
func main() {
	...
	if info.IsDir() {
		fmt.Printf("%s %s é um diretório, informe um arquivo.\n", boldred("[!]"), inputFile)
		os.Exit(1)
	}
	...
	if !isOnline {
		connStatus = red("OFFLINE (Modo Cache)")
	}
	...
	for _, lang := range targetLangs {
		...
		go func(l string) {
			...
			// Incrementa o contador global ao finalizar um idioma
			atomic.AddInt32(&langsDone, 1)
		}(lang)
	}
	wg.Wait()

	// Mostra o progresso final solicitado após o Wait
	if !quietFlag && !verboseFlag {
		fmt.Printf("\n   %s %s: %d/%d concluídos...\n", blue("→"), white("Progresso"), langsDone, totalLangs)
	}
...
func translateFile(baseName, lang string) {
	...
	for countScanner.Scan() {
		if strings.HasPrefix(countScanner.Text(), "msgid ") {
			totalLines++
		}
	}
	file.Seek(0, 0)
	...
	for scanner.Scan() {
		...
		if strings.HasPrefix(line, "msgid ") {
			...
			current++
			if !quietFlag && !verboseFlag {
				fmt.Printf("\r   %s %-7s [%d/%d] %d%% ", blue("→"), cyan(lang), current, totalLines, (current*100)/totalLines)
			}
...
func checkDependencies() {
	...
	for _, bin := range required {
		if _, err := exec.LookPath(bin); err != nil {
			log.Fatalf("Dependência faltando: %s", bin)
		}
	}
}
...
func confLog() {
	...
	if quietFlag { logger = log.New(logFile, "", 0)
	} else { logger = log.New(io.MultiWriter(os.Stdout, logFile), "", 0) }
}
