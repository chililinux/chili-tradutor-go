/*
   chili-tradutor-go - v1.5.5
   Copyright (c) 2023-2026, Vilmar Catafesta <vcatafesta@gmail.com>
   Melhoria: Normalização de Cache (Case-insensitive e Trim)
*/
...
func normalizeText(t string) string {
	// Remove aspas, espaços extras e converte para minúsculo para busca no cache
...
func translateMessage(msgid, lang, eng string) string {
	...
	// Salva no cache usando a chave normalizada
	addToCache(lang, msgidClean, finalText)

	return formatMsgstr(finalText)
}

// [Restante das funções: main, protectVars, restoreVars, formatMsgstr, etc permanecem iguais]
// (Vou omitir as repetitivas para focar no que mudou)
...
func main() {
	...
	pflag.StringSliceVarP(&languages, "language", "l", nil, "Idiomas (ou 'all')")
	pflag.Parse()

	if showVersion { fmt.Println(_VERSION_); os.Exit(0) }
	if inputFile == "" { usage(); os.Exit(1) }
	...
	if quietFlag { logOutput = fileLog } else { logOutput = io.MultiWriter(os.Stdout, fileLog) }
	...
	if len(languages) > 0 {
		if strings.ToLower(languages[0]) == "all" { targetLangs = supportedLanguages } else { targetLangs = languages }
	}

	logger.Printf("%s Extraindo strings...", black("[STEP 1]"))
	prepareGettext(inputFile)

	logger.Printf("%s Iniciando processamento de tradução", black("[STEP 2]"))
	logger.Printf("%s Idiomas selecionados: %s", magenta("[LANGS]"), cyan(len(targetLangs)))
	logger.Printf("%s Processos paralelos:  %s", magenta("[JOBS] "), cyan(jobs))
	logger.Printf("%s Motor de tradução:    %s", magenta("[ENGINE]"), cyan(engine))
	fmt.Println()
...
func restoreVars(text string, placeholders map[string]string) string {
	for ph, original := range placeholders { text = strings.Replace(text, ph, original, -1) }
	return text
}
...
func usage() {
	...
	fmt.Println("\nOpções:")
	fmt.Println("  -i, --inputfile   Arquivo fonte para tradução")
	fmt.Println("  -l, --language    Idiomas (ex: pt-BR,en) ou 'all'")
	fmt.Println("  -e, --engine      Motor: google, bing, yandex")
	fmt.Println("  -j, --jobs        Traduções simultâneas (padrão: 3)")
	fmt.Println("  -f, --force       Força nova tradução (ignora cache)")
	fmt.Println("  -I, --install     Instala os .mo em /usr/share/locale")
}
