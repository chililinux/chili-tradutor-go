...
其主要优势是智能缓存 v2.1.9，可大幅减少网络调用，并通过本地数据重用加快重复翻译速度。
...
* 渐进式界面：实时显示每种语言的进度，并具有完美的视觉对齐，无论语言代码大小如何（例如 en 与 zh-CN）。
...
辣椒-tradutor-go -i meu_script.sh
...
* Scripts/POT：在 ./pot/ 中生成 .po 文件，在 ./usr/share/locale/ 中生成 .mo 二进制文件。
...
* 自动更新：每次在缓存中找到项目时，都会更新其“上次使用”时间戳，以防止将来自动清除。
